...
class BollingerStrategy(CtpbeeApi):
    ...
    def __init__(self, name: str, code):
        ...
        self.pos = 0
        self.logger.info(
            f"布林带策略初始化 - 合约: {code}, 窗口大小: {self.window_size}, 标准差倍数: {self.num_of_std}",
            owner=self.name,
        )
    ...
    def on_bar(self, bar: BarData):
        ...
        upper_band, middle_band, lower_band = bollinger_bands(
            self.close, self.window_size, self.num_of_std
        )
        ...
        self.logger.debug(
            f"布林带计算 - 上轨: {current_upper:.2f}, 中轨: {current_middle:.2f}, 下轨: {current_lower:.2f}",
            owner=self.name,
        )

        # 价格突破下带买入，突破上带卖出
        buy_signal = (
            bar.close_price > current_lower and self.close[-2] <= previous_lower
        )
        sell_signal = (
            bar.close_price < current_upper and self.close[-2] >= previous_upper
        )

        if buy_signal:
            self.logger.info(
                f"买入信号 - 价格突破下轨: 当前价格 {bar.close_price:.2f} > 下轨 {current_lower:.2f}, 前值 {self.close[-2]:.2f} <= 前下轨 {previous_lower:.2f}",
                owner=self.name,
            )
        if sell_signal:
            self.logger.info(
                f"卖出信号 - 价格突破上轨: 当前价格 {bar.close_price:.2f} < 上轨 {current_upper:.2f}, 前值 {self.close[-2]:.2f} >= 前上轨 {previous_upper:.2f}",
                owner=self.name,
            )
